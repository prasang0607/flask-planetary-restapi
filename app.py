...
def super_simple():
    return jsonify(message='Hello from Palnetary API'), 200
...
def retrieve_password(email: str):
    ...
    if user:
        msg = Message(body=f"Your planetary API password is '{user.password}'.",
                      subject='Planetary API',
                      sender='admin@planetary-api.com',
                      recipients=[user.email])
        mail.send(msg)
        return jsonify(message=f'Password sent to {user.email}.')
...
def planet_details(planet_id: int):
    ...
    if planet:
        ...
        return jsonify(result)
    return jsonify(message=f"planet with id {planet_id} doesn't exist."), 404
